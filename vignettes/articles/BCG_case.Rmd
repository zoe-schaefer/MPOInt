...
CRAN_pkgs <- c("stringr", "tibble", "ggvenn", "devtools",
               ...
               "ggrepel", "scales", "htmltools", "dplyr")
...
cat(paste("**NMIBC/Phospho:** ", paste(sort(intersect(GDA_BlCa$gene_symbol, phospho_FC$SYMBOL2)), collapse = ", ")))
cat(paste("**NMIBC/Total:** ", paste(sort(intersect(GDA_BlCa$gene_symbol, total_FC$SYMBOL2)), collapse = ", ")))
cat(paste("**Total/Phospho:** ", paste(sort(intersect(total_FC$SYMBOL2, phospho_FC$SYMBOL2)), collapse = ", ")))
cat(paste("**NMIBC/Phospho/Total:** ", paste(sort(intersect(intersect(GDA_BlCa$gene_symbol, phospho_FC$SYMBOL2), total_FC$SYMBOL2)), collapse = ", ")))
```

```{r GDA-list-melanoma, results = "asis", collapse = TRUE}
cat(paste("**Melanoma/Phospho:** ", paste(sort(intersect(GDA_melanoma$gene_symbol, phospho_FC$SYMBOL2)), collapse = ", ")))
cat(paste("**Melanoma/Total:** ", paste(sort(intersect(GDA_melanoma$gene_symbol, total_FC$SYMBOL2)), collapse = ", ")))
cat(paste("**Melanoma/Phospho:** ", paste(sort(intersect(total_FC$SYMBOL2, phospho_FC$SYMBOL2)), collapse = ", ")))
cat(paste("**Melanoma/Phospho/Total:** ", paste(sort(intersect(intersect(GDA_melanoma$gene_symbol, phospho_FC$SYMBOL2), total_FC$SYMBOL2)), collapse = ", ")))
```

```{r GDA-list-RSV, results = "asis", collapse = TRUE}
cat(paste("**RSV/Phospho:** ", paste(sort(intersect(GDA_RSV$gene_symbol, phospho_FC$SYMBOL2)), collapse = ", ")))
cat(paste("**RSV/Total:** ", paste(sort(intersect(GDA_RSV$gene_symbol, total_FC$SYMBOL2)), collapse = ", ")))
cat(paste("**Total/Phospho:** ", paste(sort(intersect(total_FC$SYMBOL2, phospho_FC$SYMBOL2)), collapse = ", ")))
cat(paste("**RSV/Phospho/Total:** ", paste(sort(intersect(intersect(GDA_RSV$gene_symbol, phospho_FC$SYMBOL2), total_FC$SYMBOL2)), collapse = ", ")))
```
...
```{r clear-4}
# Clearing large variables
rm(GDA_BlCa, GDA_melanoma, GDA_RSV)
```
...
edges_df <- left_join(complex_vertices, edges_df, by = join_by("vertices" == "Symbol")) %>% dplyr::rename("Symbol" = "vertices")
...
ggplot(sig_network, aes(x = Complex, y = Symbol, fill = PTM)) +
  geom_bin_2d(binwidth = 1, center = 0) +
...
```{r complex-PTMs}
# Differential PTMs
final_PTMs <- distinct(final_complexes, pick(PTM, Diff)) %>% dplyr::rename("Differential expression" = "Diff")
...
vertices_df$Colors <- case_when(vertices_df$Source == "Total" ~ "red",
                          vertices_df$Source == "Phospho" ~ "blue",
                          vertices_df$Source == "Histone" ~ "gray")
